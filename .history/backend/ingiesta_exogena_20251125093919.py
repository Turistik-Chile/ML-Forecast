...
# ERA5-Land suele traer snowfall en 0/None para muchos puntos.
# ERA5 trae variables completas.
...
def ensure_configuracion(conn: pyodbc.Connection, nombre: str) -> str:
    """
    Tu tabla ia.configuracion NO tiene 'id'.
    Devuelve code_id (nvarchar). Si existe fila con ese nombre, reutiliza su code_id.
    """
    ...
    with conn.cursor() as cur:
        # si existe code_id, lo usamos como PK lógica
        if has_code_id:
            cur.execute(
            ...
            # extremo: si no tiene code_id, igual evitamos romper
            cur.execute(
                "SELECT nombre FROM ia.configuracion WHERE nombre = ?",
                (nombre,),
            )
            row = cur.fetchone()
            if row:
                logging.info(f"Configuración ya existe: {nombre} (sin code_id)")
        ...
        # recuperamos el code_id insertado
        if has_code_id:
            cur.execute(
                "SELECT code_id FROM ia.configuracion WHERE nombre = ?",
                (nombre,),
