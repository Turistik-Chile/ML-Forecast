...
def ensure_row_by_nombre(
    ...
    Devuelve PK real (id o code_id).

    FIXES:
    - Si existe columna code_id, inserta uno SIEMPRE (porque es NOT NULL en tu schema real).
    - Si PK es id pero no es identity, SCOPE_IDENTITY() puede ser NULL -> reselect por nombre.
    """
    ...
    with conn.cursor() as cur:
        ...
        # ✅ SIEMPRE setear code_id si existe en la tabla
        new_code_id = None
        if "code_id" in cols:
        ...
        # ✅ Si no hay identity, reconsultar por nombre
        if new_id is None:
            cur.execute(f"SELECT id FROM {SCHEMA}.{table} WHERE nombre = ?", (nombre,))
            new_id = cur.fetchone()[0]
...
def insert_fecha_valor_bulk(
    ...
    if exogena_id is None:
        raise ValueError(
            "exogena_id llegó None. Revisa ensure_row_by_nombre / PK real."
        )
...
def run_pipeline(
    ...
    try:
        ...
        # 3) DUMMIE espejo para FK de fecha_valor_exogena
        ...
        if exog_dummie_pk is None:
            raise ValueError("exog_dummie_pk quedó None. No se puede insertar fechas.")
