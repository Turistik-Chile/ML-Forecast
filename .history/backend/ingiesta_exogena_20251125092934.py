...
def _scale_snow_to_mm(
    data_json: dict, snow_values: List[Optional[float]]
) -> List[float]:
    """
    Convierte snowfall_sum a mm de nieve (o equivalente) según daily_units.
    Así evitamos que int(round()) te deje todo en 0.
    """
    unit = (
        data_json.get("daily_units", {}).get("snowfall_sum")
        or data_json.get("daily_units", {}).get("snowfall")
        or ""
    ).lower()
    ...
    logging.info(f"Unidad snowfall_sum='{unit}' -> factor_to_mm={factor}")
...
def fetch_snow_archive_daily(
    ...
    snow_raw = data.get("daily", {}).get("snowfall_sum", [])

    snow_mm = _scale_snow_to_mm(data, snow_raw)

    out = []
    for t, s_mm in zip(times, snow_mm):
