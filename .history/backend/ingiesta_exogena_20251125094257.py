...
# ERA5-Land suele devolver snowfall en 0/None. ERA5 trae set completo.
...
def ensure_configuracion(conn: pyodbc.Connection, nombre: str) -> str:
    """
    ia.configuracion NO tiene 'id'. Usamos code_id como PK lógica.
    Devuelve code_id (nvarchar).
    """
    cols = get_table_columns(conn, "configuracion")
    has_code_id = "code_id" in cols
    has_comentarios = "comentarios" in cols

    with conn.cursor() as cur:
        if has_code_id:
            cur.execute(
                "SELECT code_id FROM ia.configuracion WHERE nombre = ?",
                (nombre,),
            )
            row = cur.fetchone()
            if row and row[0]:
                logging.info(f"Configuración ya existe: {nombre} (code_id={row[0]})")
                return str(row[0])

        insert_cols = ["nombre"]
        insert_vals: List[Any] = [nombre]
        ...
        if has_code_id:
            ...
            insert_vals.append(f"cfg_{uuid4().hex}")  # para local da igual
        ...
        if has_code_id:
            ...
            new_code_id = cur.fetchone()[0]
            logging.info(f"Configuración creada: {nombre} (code_id={new_code_id})")
            return str(new_code_id)

        logging.info(f"Configuración creada: {nombre} (sin code_id)")
        return nombre


def ensure_variable_exogena_normal(
    conn: pyodbc.Connection, nombre: str, configuracion_id: Any
) -> int:
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT id FROM ia.variable_exogena WHERE nombre=? AND configuracion_id=?",
        ...
        insert_vals: List[Any] = [nombre, 1, configuracion_id]

        new_code = None
        if has_code_id:
            new_code = f"exg_{uuid4().hex}"
            insert_cols.append("code_id")
            insert_vals.append(new_code)
        ...
        # SIN SCOPE_IDENTITY: recuperamos por clave lógica
        if has_code_id and new_code is not None:
            cur.execute(
                "SELECT id FROM ia.variable_exogena WHERE code_id=?",
                (new_code,),
            )
        else:
            cur.execute(
                "SELECT TOP 1 id FROM ia.variable_exogena "
                "WHERE nombre=? AND configuracion_id=? "
                "ORDER BY id DESC",
                (nombre, configuracion_id),
            )

        exog_id = cur.fetchone()[0]
...
def ensure_variable_exogena_dummie(
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT id FROM ia.variable_exogena_dummie WHERE nombre=? AND configuracion_id=?",
            (nombre, configuracion_id),
        )
        row = cur.fetchone()
        if row:
        ...
        if has_code_id and new_code is not None:
            cur.execute(
                "SELECT id FROM ia.variable_exogena_dummie WHERE code_id=?",
                (new_code,),
            )
        else:
            cur.execute(
                "SELECT TOP 1 id FROM ia.variable_exogena_dummie "
                "WHERE nombre=? AND configuracion_id=? "
                "ORDER BY id DESC",
                (nombre, configuracion_id),
            )
...
def run_pipeline(
    ...
    conn = get_connection()
    try:
        # 1) Configuración (devuelve code_id)
        config_code_id = ensure_configuracion(conn, CONFIG_NOMBRE)

        # 2) Variable exógena NORMAL
        exog_normal_id = ensure_variable_exogena_normal(
            conn, EXOGENA_NOMBRE, config_code_id
        )

        # 3) Variable espejo DUMMIE
        exog_dummie_id = ensure_variable_exogena_dummie(
            conn, EXOGENA_NOMBRE, config_code_id
        )
        ...
        logging.info(
            f"Pipeline OK. config_code_id={config_code_id}, "
