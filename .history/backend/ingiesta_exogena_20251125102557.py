...
def ensure_variable_exogena_normal(
    ...
    """
    Asegura una única variable exógena NORMAL en ia.variable_exogena.
    Devuelve su id (int).
    """
...
def fetch_existing_dates(conn: pyodbc.Connection, exogena_id: int) -> set:
    """
    Trae fechas ya registradas para esa exógena (NORMAL).
    """
    with conn.cursor() as cur:
        cur.execute(
...
def insert_fecha_valor_bulk(
    conn: pyodbc.Connection, exogena_id: int, items: List[Tuple[date, int]]
):
    if not items:
...
def run_pipeline(
    ...
    try:
        ...
        logging.info(
            f"Pipeline OK. config_code_id={config_code_id}, exog_normal_id={exog_normal_id}"
        )
    finally:
        conn.close()
